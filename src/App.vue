...
async function handleSend(text) {
  ...
  if (hasDeepseek.value) {
    ...
    if (dsResult.error) {
      ...
      // Determine tracking params: prefer AI JSON, fallback to regex parse
      let qTn = null, qCc = '', qPh = ''
      if (aiTracking && aiTracking.trackingNumber) {
        qTn = aiTracking.trackingNumber; qCc = aiTracking.carrierCode || ''; qPh = aiTracking.phone || ''
      } else if (parsed && parsed.type === 'query') {
        qTn = parsed.tracking_number; qCc = parsed.carrier_code || ''; qPh = parsed.phone
      }

      if (qTn) {
        const trackIdx2 = messages.length
        messages.push({ id: ++msgId, role: 'system', type: 'thinking' })
        const aiResult = await queryTracking(keys.uapi, { tracking_number: qTn, carrier_code: qCc, phone: qPh }, token.value, usingGlobal.value)
        const trackMsg2 = messages[trackIdx2]

        if (aiResult.error) {
          const errMsg = getErrorMessage(aiResult.status, aiResult.message, aiResult.code)
          messages.splice(trackIdx2, 1)
          const explainPrompt = `查询快递单号 ${qTn} 时失败了：${errMsg.replace(/<[^>]+>/g, '')}。请用1-2句话友好地告诉用户原因和建议。`
